...
sed -e "s:ODDBALL1DIR:${oddball1feat_dir}:g" "${src_dir}"/design-template-GF-old-oddball-session-higherlevel.fsf \
    > "${out_dir}"/design.fsf
sed -i -e "s:ODDBALL2DIR:${oddball2feat_dir}:g" "${out_dir}"/design.fsf
sed -i -e "s:OUTDIR:${out_dir}/GF-old-oddball-12:g" "${out_dir}"/design.fsf
