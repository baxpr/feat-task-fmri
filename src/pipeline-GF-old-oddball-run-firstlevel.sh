...
feat_dir="${out_dir}/feat-GF-old-oddball"
mkdir -p "${feat_dir}"
...
parse-edat-GF-old-oddball.py --eprime_csv "${out_dir}"/eprime.csv --feat_dir "${feat_dir}"
...
sed -e "s:SUBJDIR:${feat_dir}:g" "${src_dir}"/design-template-GF-old-oddball-run-firstlevel.fsf \
    > "${feat_dir}"/design.fsf
...
setup-reg.sh --std_niigz "${meanfmri_niigz}" --feat_dir "${feat_dir}"/GF-old-oddball.feat

# Copy html to a different location to be a separate output
echo Copying HTML
mkdir "${out_dir}"/HTML
cp -R "${feat_dir}"/GF-old-oddball.feat/*.html "${out_dir}"/HTML
cp -R "${feat_dir}"/GF-old-oddball.feat/*.png "${out_dir}"/HTML
cp -R "${feat_dir}"/GF-old-oddball.feat/.files "${out_dir}"/HTML
