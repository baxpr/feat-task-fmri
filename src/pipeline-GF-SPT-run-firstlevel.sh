#!/usr/bin/env bash
#
# Main entrypoint for FSL/FEAT first level stats for GF SPT task
#
...
setup-reg.sh --std_niigz "${meanfmri_niigz}" --feat_dir "${feat_dir}"/GF-SPT.feat

# Copy html to a different location to be a separate output
echo Copying HTML
mkdir "${out_dir}"/HTML
cp -R "${feat_dir}"/GF-SPT.feat/*.html "${out_dir}"/HTML
cp -R "${feat_dir}"/GF-SPT.feat/*.png "${out_dir}"/HTML
cp -R "${feat_dir}"/GF-SPT.feat/.files "${out_dir}"/HTML
